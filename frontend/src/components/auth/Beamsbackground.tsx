...
function createBeam(width: number, height: number): Beam {
  ...
  return {
    ...
    length: height * 2.5,
    angle: angle,
    speed: 1.2 + Math.random() * 2.4, // Increased base speed and randomness
...
export default function BeamsBackground({ className, intensity = "strong" }: AnimatedGradientBackgroundProps) {
  ...
  const MINIMUM_BEAMS = 15 // Reduced number of beams
  ...
  useEffect(() => {
    ...
    const updateCanvasSize = () => {
      const dpr = Math.min(2, window.devicePixelRatio || 1); // Limit devicePixelRatio
    ...
    function resetBeam(beam: Beam, index: number, totalBeams: number) {
      ...
      beam.speed = 1.0 + Math.random() * 0.8; // Slightly increased reset speed
    ...
    function animate() {
      ...
      ctx.filter = "blur(30px)" // Slightly reduced blur radius
  ...
  return (
    <div className={cn("relative min-h-screen w-full overflow-hidden bg-neutral-950", className)}>
      <canvas ref={canvasRef} className="absolute inset-0" style={{ filter: "blur(10px)" }} /> {/* Reduced canvas blur */}
